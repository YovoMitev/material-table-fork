...
function (_React$Component) {
  ...
  (0, _createClass2["default"])(MaterialTable, [{
    ...
    value: function componentDidUpdate(prevProps, prevState) {
      ...
      if (this.props.detailPanel && typeof this.props.detailPanel !== 'function' && !(0, _fastDeepEqual["default"])(prevProps.detailPanel[0].openedRows, this.props.detailPanel[0].openedRows)) {
        this.toggleOpenedRows();
        return;
      }

      if (!(0, _fastDeepEqual["default"])(prevState.originalData, this.state.originalData)) {
        this.toggleOpenedRows(true);
        return;
      }
    ...
    value: function hideAllRows() {
      ...
      this.state.renderData.map(function (data, index) {
        if (data && data.tableData && data.tableData.showDetailPanel && typeof data.tableData.showDetailPanel === 'function') {
          var render = _this3.props.detailPanel && _this3.props.detailPanel[0].render;
          var path = [index + _this3.state.pageSize * _this3.state.currentPage];

          _this3.onToggleDetailPanel(path, render);
        }
      });

      if (this.props.detailPanel && this.props.detailPanel[0]) {
        this.props.detailPanel[0].hideAllRows();
      }
    }
  }, {
    key: "toggleOpenedRows",
    value: function toggleOpenedRows() {
      var _this4 = this;

      var forceUpdate = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;

      if (this.state.renderData && this.state.renderData.length > 0 && this.props.detailPanel && typeof this.props.detailPanel !== 'function' && this.props.detailPanel.length > 0 && this.props.detailPanel[0].openedRows.length > 0) {
        this.state.renderData.map(function (data, index) {
          _this4.props.detailPanel.map(function (panel) {
            if (typeof panel === 'function') {
              return;
            }

            var path = [index + _this4.state.pageSize * _this4.state.currentPage];

            if (panel.openedRows && data.tableData && (forceUpdate || data.tableData && !data.tableData.showDetailPanel) && panel.openedRows.includes(data.tableData.id)) {
              _this4.onToggleDetailPanel(path, panel.render);
            }
          });
        });
      }
        ...
        if (calculatedProps.editable.onRowAdd) {
          calculatedProps.actions.push({
            ...
            onClick: function onClick() {
              ...
              _this5.setState((0, _objectSpread2["default"])({}, _this5.dataManager.getRenderState(), {
          ...
          calculatedProps.actions.push(function (rowData) {
            return {
              ...
              onClick: function onClick(e, rowData) {
                _this5.dataManager.changeRowEditing(rowData, 'update');
        ...
        if (calculatedProps.editable.onRowDelete) {
          calculatedProps.actions.push(function (rowData) {
            return {
              ...
              onClick: function onClick(e, rowData) {
                _this5.dataManager.changeRowEditing(rowData, 'delete');
    ...
    value: function render() {
      ...
      return React.createElement(_reactBeautifulDnd.DragDropContext, {
        ...
        var table = _this6.renderTable(props);

        return React.createElement("div", {
          ref: provided.innerRef
        }, React.createElement("div", {
          ref: _this6.tableContainerDiv,
          style: {
            maxHeight: props.options.maxBodyHeight,
            minHeight: props.options.minBodyHeight,
            overflowY: props.options.overflowY
          }
        }, _this6.state.width && props.options.fixedColumns && props.options.fixedColumns.right ? React.createElement("div", {
          style: {
            width: _this6.getColumnsWidth(props, -1 * props.options.fixedColumns.right),
            position: 'absolute',
            top: 0,
            right: 0,
          ...
          style: {
            width: _this6.state.width,
            background: 'white',
            transform: "translateX(calc(".concat(_this6.getColumnsWidth(props, -1 * props.options.fixedColumns.right), " - 100%))")
          }
        }, table)) : null, React.createElement("div", null, table), _this6.state.width && props.options.fixedColumns && props.options.fixedColumns.left ? React.createElement("div", {
          style: {
            width: _this6.getColumnsWidth(props, props.options.fixedColumns.left),
            position: 'absolute',
            top: 0,
            left: 0,
            boxShadow: '2px 0px 15px rgba(125,147,178,.25)',
