...
export default class MaterialTable extends React.Component {
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (
      ...
      typeof this.props.detailPanel !== 'function' &&
      !equal(
        prevProps.detailPanel[0].openedRows,
        this.props.detailPanel[0].openedRows
      )
    ) {
      this.toggleOpenedRows()
      return
    ...
    if (!equal(prevState.originalData, this.state.originalData)) {
  ...
  hideAllRows () {
    this.state.renderData.map((data, index) => {
      if (
        ...
        typeof data.tableData.showDetailPanel === 'function'
      ) {
        const render =
          this.props.detailPanel && this.props.detailPanel[0].render
        const path = [index + this.state.pageSize * this.state.currentPage]
        this.onToggleDetailPanel(path, render)
      }
    })
  ...
  toggleOpenedRows (forceUpdate = false) {
    if (
      this.state.renderData &&
      this.state.renderData.length > 0 &&
      this.props.detailPanel &&
      typeof this.props.detailPanel !== 'function' &&
      this.props.detailPanel.length > 0 &&
      this.props.detailPanel[0].openedRows.length > 0
    ) {
      this.state.renderData.map((data, index) => {
        this.props.detailPanel.map(panel => {
          ...
          if (
            ...
            panel.openedRows.includes(data.tableData.id)
          ) {
            this.onToggleDetailPanel(path, panel.render)
          }
        })
      })
    }
        ...
        <props.components.Header
          ...
          dataCount={
            props.parentChildData
              ...
              : this.state.columns.filter(col => col.tableData.groupOrder > -1)
                  .length > 0
